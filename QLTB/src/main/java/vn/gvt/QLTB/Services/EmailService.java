...
public class EmailService {
    ...
    @Scheduled(cron = "0 0 7,11,17 * * *") // Tự động gửi mail lúc 7h 12h 17h
    public void sendOverdueEmails() throws MessagingException {
        ...
        for (PhieuMuon phieuMuon : allPhieuMuon) {
            if (phieuMuon.getHanTra().isBefore(now) && phieuMuon.getThoiDiemTra() == null) {
                for (CTPhieuMuon ctPhieuMuon : phieuMuon.getListCTPhieuMuon()) {
                    NguoiMuon nguoiMuon = ctPhieuMuon.getNguoiMuon();
                    String email = nguoiMuon.getEmail();

                    String ctPhieuMuonContent = "";
                    int i = 0;
                    for (CTPhieuMuon ctpm : phieuMuon.getListCTPhieuMuon()) {
                        if (ctpm.getTrangThai().getMaTinhTrang() == 3 && ctpm.getNguoiMuon().getMaNguoiMuon() == nguoiMuon.getMaNguoiMuon()) {
                    ...
                    emailSender.sendEmail(email, "Phiếu mượn quá hạn", message);
                }
            }
        }
    }
